...
function moveEnemies() {
    constants_1.enemies.forEach(enemy => {
        ...
        enemy.y = (enemy.y + constants_1.PVP_WORLD_HEIGHT) % constants_1.PVP_WORLD_HEIGHT;
    });
...
function createObstacle() {
    ...
    return {
        id: Math.random().toString(36).substr(2, 9),
        x: Math.random() * constants_1.PVP_WORLD_WIDTH,
        y: Math.random() * constants_1.PVP_WORLD_HEIGHT,
...
function respawnPlayer(player) {
    ...
    player.x = spawnX;
    player.y = Math.random() * constants_1.PVP_WORLD_HEIGHT;
...
io.on('connection', (socket) => {
    ...
    socket.on('authenticate', async (credentials) => {
        ...
        if (user) {
            ...
            constants_1.players[socket.id] = {
                ...
                x: 200,
                y: constants_1.PVP_WORLD_HEIGHT / 2,
                angle: 0,
                score: 0,
                velocityX: 0,
                velocityY: 0,
    ...
    socket.on('playerMovement', (movementData) => {
        ...
        if (player) {
            ...
            for (const otherPlayerId in constants_1.players) {
                if (otherPlayerId === socket.id)
                    continue; // Skip self
                const otherPlayer = constants_1.players[otherPlayerId];
                // Check if players are colliding
                if (newX < otherPlayer.x + constants_1.PLAYER_SIZE &&
                    newX + constants_1.PLAYER_SIZE > otherPlayer.x &&
                    newY < otherPlayer.y + constants_1.PLAYER_SIZE &&
                    newY + constants_1.PLAYER_SIZE > otherPlayer.y) {
                    collision = true;
                    if (!player.isInvulnerable && !otherPlayer.isInvulnerable) {
                        ...
                        newX -= normalizedDx * constants_1.KNOCKBACK_FORCE;
                        newY -= normalizedDy * constants_1.KNOCKBACK_FORCE;
                        otherPlayer.x += normalizedDx * constants_1.KNOCKBACK_FORCE;
                        otherPlayer.y += normalizedDy * constants_1.KNOCKBACK_FORCE;
                        // Store knockback for gradual recovery
                        player.knockbackX = -normalizedDx * constants_1.KNOCKBACK_FORCE;
                        player.knockbackY = -normalizedDy * constants_1.KNOCKBACK_FORCE;
                        otherPlayer.knockbackX = normalizedDx * constants_1.KNOCKBACK_FORCE;
                        otherPlayer.knockbackY = normalizedDy * constants_1.KNOCKBACK_FORCE;
            ...
            player.x = Math.max(0, Math.min(constants_1.PVP_WORLD_WIDTH - constants_1.PLAYER_SIZE, newX));
            player.y = Math.max(0, Math.min(constants_1.PVP_WORLD_HEIGHT - constants_1.PLAYER_SIZE, newY));
